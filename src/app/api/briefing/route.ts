...
export async function GET() {
  ...
  // Check token budget before AI call
...
今天天气：${weather.city ? `${weather.city}，` : ""}${weather.condition} ${weather.temp}`,
      ...
      if (res.ok) {
        ...
        // Record token usage
        if (data.usage) {
          record(data.usage.prompt_tokens || 0, data.usage.completion_tokens || 0);
        }
      }
    } catch (e) {
    ...
    console.warn("Token budget exceeded, skipping AI briefing generation");
  }
